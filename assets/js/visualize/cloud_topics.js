...
function draw_cloud_topics(areaID, repoNameWOwner) {
    ...
    function drawCloud(data, areaID) {
        ...
        var element = document.querySelector('.' + areaID);
        if (!element) {
            return;
        }
        ...
        var margin = { top: stdMargin.top, right: stdMargin.right / 2, bottom: stdMargin.bottom / 2, left: stdMargin.left / 2 },
            ...
            width = Math.min(element.offsetWidth, 500) - margin.left - margin.right,
            height = Math.min(element.offsetWidth, 450)  - margin.top - margin.bottom;
